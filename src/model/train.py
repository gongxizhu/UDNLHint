...
from model import model as nmt_model
...
def check_stats(stats, global_step, steps_per_stats, hparams, log_f):
    ...
    utils.print_out(
        "  global step %d lr %g "
        "step-time %.2fs wps %.2fK ppl %.2f %.2f %.2f gN %.2f %s" %
        (global_step, stats["learning_rate"],
         avg_step_time, speed, train_ppl, stats["loss"], stats["predict_count"], avg_grad_norm,
...
def train(hparams, scope=None, target_session=""):
    ...
    while global_step < num_train_steps:
        ...
        global_step = update_stats(
            stats, summary_writer, start_time, step_result)
