...
with open(preprocessing['protected_phrases_file'], 'r', encoding='utf-8') as protected_file:
    protected_phrases_regex = list(filter(lambda word: False if word[0] == '#' else True, filter(
        None, protected_file.read().split("\n"))))
...
def tokenize(sentence):
    ...
    if m:
        ...
        for dots in list(set(m)):
            sentence = sentence.replace(
                dots, ' PROTECTEDPERIODS{}PROTECTEDPERIODS '.format(len(dots)))
    ...
    sentence = []
    words = prepare_text_for_lda(words)
    # For every word
    for word in words:
    ...
    if protected_phrases_counter:
        sentence = regex['restorephrases'].sub(lambda number: protected_phrases_replace[int(
            number.group(1).replace(" ", ""))], sentence)
    if protected_periods_counter:
        sentence = regex['restoreperiods'].sub(lambda number: (
            "." * int(number.group(1).replace(" ", ""))), sentence)
...
def replace(entity):
    ...
    replacement = entity.group(0).replace(
        phrase, ' PROTECTEDREGEXPHRASE{}PROTECTEDREGEXPHRASE '.format(protected_phrases_counter))
...
with open(preprocessing['answers_detokenize_file'], 'r', encoding='utf-8') as answers_detokenize_file:
    answers_detokenize_regex = list(filter(lambda word: False if word[0] == '#' else True, filter(
        None, answers_detokenize_file.read().split("\n"))))
            ...
            if re.search(detokenize_regex, answer):
                ...
                for p in regex.finditer(answer):
                    ...
                    if len(p.groups()) > 1:
                        ...
                        for i, group in enumerate(groups):
                            ...
                            answer = answer[:position] + answer[position:].replace(
                                " ", "##DONOTTOUCHTHISSPACE##", 1)

                        # Update reges to match placeholders as spaces
                        detokenize_regex = detokenize_regex.replace(
                            ' ', '(?: |##DONOTTOUCHTHISSPACE##)')
                ...
                for p in regex.finditer(answer):
                    ...
                    answer = answer[:position] + \
                        answer[position:].replace(replace_from, replace_to, 1)
...
def prepare_text_for_lda(words):
    tokens = [token for token in words if token not in en_stop]
